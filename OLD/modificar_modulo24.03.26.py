...
def _en_zona(e):
    ...
    capas_borrar_siempre = {
        'PL-LIM-CASETA', 'PL-PILARES', 'CORREAS', 'CARRIL',
...
# offset Y carril vertical: depende del ancho del módulo (medido del ala interior del pilar)
...
def dibujar_carriles(msp, x0, y0, x1, y1, grosor, ancho_modulo=2350):
    ...
    ofs_v_y = CARRIL_OFS_V_Y_MAP.get(ancho_modulo, 170)
    cy_ini = y0 + ofs_v_y
    cy_fin = y1 - ofs_v_y
...
def _munon_pilar_str(A, long_pilar):
    """Devuelve (texto_munon, texto_pilar) según ancho módulo y longitud de pilar."""
    # Dimensiones según ancho A: exterior del pilar
    # Valores estándar: A=2350 -> 195x167, A=2400 -> 220x192, A=2440 -> 220x192
    if   A <= 2350: px, py = 195, 167
    elif A <= 2400: px, py = 220, 192
    else:           px, py = 220, 192
...
def dibujar_alzado_base(msp, doc, x0, y0, x1, y1, hbase, correas, tipo_tablero, long_pilar, A, g_carril):
    ...
    # Línea horizontal desde x1 hasta la primera correa (tapa del alzado)
    if correas:
        msp.add_line((x1, alz_cer_insert_y), (x0 + correas[0] - 25.0, alz_cer_insert_y),
                     dxfattribs={'layer': 'ALZ-CER-DIB'})

    # ── C. Líneas de eje verticales dentro del módulo (Ejes) ──────────────────
    for pos in correas:
        msp.add_line((x0+pos, y0), (x0+pos, y1),
                     dxfattribs={'layer': 'Ejes', 'color': 8})

    # ── D. Cotas horizontales entre correas encima (PMP-T-60) ─────────────────
    puntos_cx = [x0] + [x0 + p for p in correas] + [x1]
    dim_y_cor = y1 + _COTA_COR_DY
    for i in range(len(puntos_cx) - 1):
        xa, xb = puntos_cx[i], puntos_cx[i+1]
        dim = msp.add_linear_dim(
            base=(xa, dim_y_cor), p1=(xa, y1), p2=(xb, y1),
            angle=0, dimstyle='PMP-T-60', dxfattribs={'layer': 'Cotas'})
        dim.render()
    ...
    # ── F. Rectángulo redondeado PERFIL BASE ──────────────────────────────────
    # Posición: siempre pegado al borde derecho (x1), ancho fijo 1038mm
    prb_ancho = 1038.0
    prb_x0 = x1 - prb_ancho
    prb_x1 = x1
    prb_y0 = y1 + _PRB_DY_BOT
    prb_y1 = y1 + _PRB_DY_TOP
    _rect_redondeado(msp, prb_x0, prb_y0, prb_x1, prb_y1,
                     radio=40, layer='Cotas', color=256)

    # Texto "PERFIL BASE Xmm" dentro del rectángulo
    msp.add_text(f'PERFIL BASE {hbase}mm',
                 dxfattribs={
                     'layer': 'TEXTO',
                     'height': 64.3,
                     'insert': (x0 + _PRB_TXT_DX, y1 + _PRB_TXT_DY),
                 })

    # ── G. Cajas MUÑONES y PILARES (LWPOLYLINE color=5 azul) ──────────────────
    for dx0, dx1 in [(_CAJA_MUN_DX0, _CAJA_MUN_DX1),
                     (_CAJA_PIL_DX0, _CAJA_PIL_DX1)]:
        msp.add_lwpolyline(
            [(x0+dx0, y1+_CAJA_DY_BOT), (x0+dx1, y1+_CAJA_DY_BOT),
             (x0+dx1, y1+_CAJA_DY_TOP), (x0+dx0, y1+_CAJA_DY_TOP)],
            close=True, dxfattribs={'layer': 'TEXTO', 'color': 5})

    # ── H. Textos dinámicos encima ────────────────────────────────────────────
    str_mun, str_pil = _munon_pilar_str(A, long_pilar)
    tipo_txt = 'FENÓLICO' if tipo_tablero == 'Fenólico' else 'HIDRÓFUGO'

    textos_encima = [
        (_TXT_MUN_DX,  _TXT_MUN_DY,  77.8,  f' {str_mun}'),
        (_TXT_PIL_DX,  _TXT_PIL_DY,  77.8,  str_pil),
        (_TXT_CHK1_DX, _TXT_CHK1_DY, 82.9,  '*COMPROBAR LA MEDIDA DEL TABLERO ANTES DE SOLDAR'),
        (_TXT_CHK2_DX, _TXT_CHK2_DY, 82.9,  '*COMPROBAR LAS DIMENSIONES DE LOS MUÑONES CON LOS PILARES'),
        (_TXT_DIST_DX, _TXT_DIST_DY, 82.9,  f'DISTRIBUCIÓN CORREAS BASE C/ {tipo_txt}'),
    ]
    for dx, dy, h, txt in textos_encima:
        msp.add_text(txt, dxfattribs={
            'layer': 'TEXTO', 'height': h,
            'insert': (x0 + dx, y1 + dy),
        })

    # ── I. Tablero: HATCH verde + contorno amarillo (centrado en módulo) ───────
    cx_mod = (x0 + x1) / 2.0
    tab_cx = cx_mod
    tab_yt = y1 + _TAB_DY_TOP   # negativo -> dentro del módulo
    tab_yb = y1 + _TAB_DY_BOT
    tab_r  = _TAB_R
    tab_semi = _TAB_DX_SEMI
    # HATCH sólido verde (color 3)
    _hatch_rect_redondeado(msp, tab_cx, tab_yt, tab_yb, tab_semi, tab_r,
                           color_hatch=3, layer_hatch='SOMBRAS')
    # Contorno amarillo (color 2)
    _rect_redondeado(msp,
                     tab_cx - tab_semi, tab_yb,
                     tab_cx + tab_semi, tab_yt,
                     radio=tab_r, layer='Cotas', color=2)
...
def dibujar_bloques_recuadros(msp, x0, y0, x1, y1, hbase, long_pilar, A,
                               panel_grosor, g_carril, panel_tipo, serie,
                               suministro, tipo_tablero, acabado):
    """
    Inserta los bloques de recuadros y anotaciones en el plano.
    Los bloques deben existir en la PLANTILLA.dxf.
    """
    cx = (x0 + x1) / 2.0

    # 1. RECUADRO H PERFIL BASE Xmm
    #    insert en (x1, y1+909.5), pegado al borde derecho
    msp.add_blockref('RECUADRO H PERFIL BASE Xmm',
                     insert=(x1, y1 + 909.5),
                     dxfattribs={'layer': 'Cotas'})

    # 2. RECUADROS MEDIDAS PILARES - MUÑONES
    #    insert en (cx, y1+1784.3)
    msp.add_blockref('RECUADROS MEDIDAS PILARES - MUÑONES',
                     insert=(cx, y1 + 1784.3),
                     dxfattribs={'layer': 'TEXTO'})

    # 3. RECUADRO TÍTULO PLANO  (distribución correas base)
    #    insert en (cx, y1+3009.6)
    msp.add_blockref('RECUADRO TÍTULO PLANO',
                     insert=(cx, y1 + 3009.6),
                     dxfattribs={'layer': 'Cotas'})

    # 4. RECUADRO NÚMERO DE UD (MÓDULOS) NUMERO SERIE
    #    insert en (x1+714.2, y1+2671.6)
    msp.add_blockref('RECUADRO NÚMERO DE UD (MÓDULOS) NUMERO SERIE',
                     insert=(x1 + 714.2, y1 + 2671.6),
                     dxfattribs={'layer': 'Cotas'})

    # 5. CARRIL PARA PANEL DE Xmm
    #    insert en (x1+477.4, y1+370.5)
    msp.add_blockref('CARRIL PARA PANEL DE Xmm',
                     insert=(x1 + 477.4, y1 + 370.5),
                     dxfattribs={'layer': 'Cotas'})

    # 6. CORTE CARRIL CARA LARGA (dos inserts enfrentados)
    #    Módulos adosados: los carriles interiores sin panel se tapan con tablero.
    #    Por ahora se insertan siempre (en el futuro: solo si suministro indica adosamiento).
    #    rot=0   → corte inferior (pegado a y0)
    msp.add_blockref('CORTE CARRIL CARA LARGA',
                     insert=(x1 + 429.0, y0),
                     dxfattribs={'layer': 'PL-CER-DIB', 'rotation': 0})
    #    rot=180, xscale=-1 → corte superior (pegado a y1, espejado)
    ...
    #    insert en (cx, y1-1373.3) → dentro del módulo
    msp.add_blockref('RECUADRO TABLERO Y SUELO',
                     insert=(cx, y1 - 1373.3),
                     dxfattribs={'layer': 'TEXTO'})
...
def generar_modulo(fila, ruta_plantilla, ruta_salida):
    ...
    MARGEN_X     = 1220.0   # margen horizontal a cada lado del módulo
    MARGEN_Y_INF = 600.0    # margen debajo del módulo (cotas inferiores + holgura)
    MARGEN_Y_SUP = 400.0    # margen sobre el texto más alto del alzado

    # Espacio necesario encima del módulo:
    # _TXT_DIST_DY = 2807mm es la distancia del top del módulo al texto más alto
    _ESPACIO_ENCIMA = 2807.0 + MARGEN_Y_SUP         # _TXT_DIST_DY(2807) + margen_sup = 3207mm
    ...
    dibujar_textos_modulo(msp, x0, y0, x1, y1, base, c("acabado"), c("serie"))
    dibujar_bloques_recuadros(msp, x0, y0, x1, y1, hbase_draw, long_pilar, A,
